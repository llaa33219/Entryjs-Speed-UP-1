...
Entry.Utils.runAsyncCurry =
    (func, time = 0) =>
    async (...args) => {
        // 스피드 최적화 모드에서는 time을 최소화
        const sleepTime = (Entry.speedOptimized || Entry.isTurbo) ? 0 : time;
        await Entry.Utils.sleep(sleepTime);
        await func(...args);
    };
