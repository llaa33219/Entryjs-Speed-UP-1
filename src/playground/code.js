...
Entry.Code = class Code {
    ...
    tick() {
        ...
        // 극강 스피드 모드에서는 모든 이벤트 디스패치 비활성화
        const shouldDispatchEvents = !Entry.speedOptimized && !Entry.hyperSpeedMode;
        const _executeEvent = shouldDispatchEvents ? _.partial(Entry.dispatchEvent, 'blockExecute') : () => {};
        const _executeEndEvent = shouldDispatchEvents ? _.partial(Entry.dispatchEvent, 'blockExecuteEnd') : () => {};

        // 극강 모드에서는 모든 executor를 한 번에 처리
        if (Entry.hyperSpeedMode) {
            for (let i = 0; i < executors.length; i++) {
                const executor = executors[i];
                if (!executor.isPause() && !executor.isEnd()) {
                    // 안전한 실행: 최대 100번까지만 실행
                    let safetyCounter = 0;
                    while (!executor.isEnd() && safetyCounter < 100) {
        ...
        if (Entry.isTurbo || Entry.speedOptimized) {
            // 스피드 모드에서는 더 많은 루프 실행
            const maxIterations = Entry.hyperSpeedMode ? 10000 : (Entry.speedOptimized ? 1000 : 10);
            let iterations = 0;
            ...
            for (let i = 0; i < loopExecutor.length; i++) {
                ...
                if (
                    ...
                    (Entry.tickTime > performance.now() - this.isUpdateTime || Entry.speedOptimized || Entry.hyperSpeedMode)
                ) {
                    i = -1;
                }
            }
